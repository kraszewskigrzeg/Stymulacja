...
nazwapliku=Label(okno, text="wpisz nazwę pliku do zapisu:")
...
def zapiszplik():
    odpowiedz=tkinter.messagebox.askquestion('zapisz', 'czy na pewno chcesz zapisać plik')
    if odpowiedz=='yes':
        #-*- coding: ISO-8859-2 -*-
        filename=NazwaPliku.get()
        plik=open(filename+'.txt','w')
        tekst=str(wyniki.get(1.0, END))
        plik.write(tekst)
        plik.close()
        tkinter.messagebox.showinfo('zapisz',"Zapisano!")
    else :
        tkinter.messagebox.showinfo('zapisz',"anulowano!")
...
zapisz=Button(okno, text="zapisz",command=zapiszplik)                                                           
...
NazwaPliku.grid(row=23, column=4, sticky=E)
zapisz.grid(row=23, column=5)
nazwapliku.grid(row=23, column=3,sticky=E)
